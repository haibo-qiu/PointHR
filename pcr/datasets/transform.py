...
class Resample(object):
    def __init__(self, num_sample_points=1200, num_points=1024):
        self.num_points = num_points
        self.num_sample_points = num_sample_points

    def __call__(self, data_dict):
        if "coord" in data_dict.keys():
            if data_dict["coord"].shape[0] > self.num_sample_points:
                data_dict["coord"] = data_dict["coord"].unsqueeze(0)
                fps_idx = tp.furthest_point_sample(data_dict["coord"], self.num_sample_points)
                fps_idx = fps_idx[:, np.random.choice(self.num_sample_points, self.num_points, False)]
                data_dict["coord"] = torch.gather(data_dict["coord"], 1, fps_idx.unsqueeze(-1).long().expand(-1, -1, data_dict["coord"].shape[-1])).squeeze()
        return data_dict


@TRANSFORMS.register_module()
class UpdateHeight(object):
    def __init__(self, gravity_dim=1):
        self.gravity_dim = gravity_dim

    def __call__(self, data_dict):
        if "coord" in data_dict.keys():
            height = data_dict["coord"][:, self.gravity_dim:self.gravity_dim+1]
...
class RandomErasing(object):
    def __init__(self, erase_ratio=0.2, erase_application_ratio=0.2, centers=3):
        """
           erase_ratio: ratio of points to be erased
           erase_application_ratio:  possibility to apply random erase aug
           centers: random [1, centers] spheres to be erased
        """
        self.erase_ratio = erase_ratio
        self.erase_application_ratio = erase_application_ratio
        self.centers = centers

    def __call__(self, data_dict):
        if random.random() < self.erase_application_ratio:
            n = data_dict["coord"].shape[0]
            total_erase = n * self.erase_ratio
            num_centers = np.random.randint(self.centers) + 1

            # achieve different size for each center
            each_erase = np.random.rand(num_centers)
            each_erase = each_erase / each_erase.sum()                           # norm to 1
            each_erase = [int(total_erase * ratio) for ratio in each_erase]

            centers = data_dict["coord"][np.random.choice(n, size=num_centers, replace=False)]
            idx_erase = []
            for c, e in zip(centers, each_erase):
                idx_erase.append(np.argsort(np.sum(np.square(data_dict["coord"] - c), 1))[:e])
            idx_erase = np.unique(np.concatenate(idx_erase))
            idx_all = np.arange(n)
            idx_crop = np.array(list(set(idx_all) - set(idx_erase)))
