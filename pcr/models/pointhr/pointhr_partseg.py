...
class Unpool(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_channels = out_channels
        self.backend = backend
        assert self.backend in ["map"]

        self.proj = nn.Sequential(nn.Linear(in_channels, out_channels, bias=bias),
                                  PointBatchNorm(out_channels),
                                  nn.ReLU(inplace=True))

    def forward(self, points, cluster=None):
        coord, feat, offset = points
        if self.backend == "map" and cluster is not None:
            cluster = cluster['cluster'] if type(cluster) == dict else cluster
            feat = self.proj(feat)[cluster]
            ...
            # feat = pointops.interpolation(coord, skip_coord, self.proj(feat), offset, skip_offset)
            raise ValueError("only support map unpool!")
        return [coord, feat, offset]


class UnpoolV2(nn.Module):
    """
    Map Unpooling
    """
        ...
        for i in range(num_streams):
            ...
            for j in range(num_streams):
                ...
                if j > i:
                    ...
                    for k in range(j-i):
                        layers.append(UnpoolV2(in_channels, out_channels, proj=(k==0)))
                elif j == i:
                    layers = nn.Identity()
                else:
                    layers = nn.ModuleList()
...
class DecoderV2(nn.Module):
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.ups = self._upsampling(num_stream)
        self.act = nn.ReLU(inplace=True)

        self.blocks = BlockSequence(
            depth=depth,
            embed_channels=in_channels[0],
            groups=groups,
            neighbours=neighbours,
            qkv_bias=qkv_bias,
            pe_multiplier=pe_multiplier,
    ...
    def _upsampling(self, num_streams):
        up_list = nn.ModuleList()
        up_list.append(nn.Identity())   # for the ori resolution feature
        for i in range(1, num_streams+1):
            layer_list = []
            for k in reversed(range(i)):
                layer_list.append(Unpool(self.in_channels[k+1], self.in_channels[k]))
            up_list.append(nn.Sequential(*layer_list))
    ...
    def forward(self, input):
        x_list, lookup = input
        assert len(x_list) == len(self.ups)
        results = []
        for i in range(len(x_list)):
            out = x_list[i]
            if i > 0:
                for j in range(i):
                    out = self.ups[i][j](out, lookup[(i-j-2, i-j-1)])  # design -1 as the ori resolution
                results.append(out[1])      # only works on feature
            else:
                results.append(self.ups[i](out)[1])
        y = self.act(torch.stack(results).sum(0))
        points = [x_list[0][0], y, x_list[0][-1]]
        return self.blocks(points)
...
class ClsEmbed(nn.Module):
    def __init__(self,
                 embed_channels,
                 num_shape_classes
                 ):
        super(ClsEmbed, self).__init__()
        self.embed_channels = embed_channels
        ...
        self.mlp_cls = nn.Sequential(
            nn.Linear(num_shape_classes, embed_channels),
            nn.ReLU(inplace=True)
        )
        self.mlp_fuse = nn.Sequential(
            nn.Linear(3*embed_channels, embed_channels),
            PointBatchNorm(embed_channels),
            nn.ReLU(inplace=True)
        )

    def forward(self, points, cls):
        coord, feat, offset = points
        feat_tmp = []
        for i in range(offset.shape[0]):
            if i == 0:
                start, end, count = 0, offset[0], offset[0]
            else:
                start, end, count = offset[i-1], offset[i], offset[i] - offset[i-1]
            x = feat[start:end, :]
            y = torch.eye(self.num_shape_classes, device=coord.device)[cls[i].long(), ] # one_hot
            x = torch.cat((
                x, self.mlp_feat(x.mean(0, True)).repeat(count, 1),
                self.mlp_cls(y.unsqueeze(0)).repeat(count, 1)), dim=1
                )
            feat_tmp.append(x)
        feat = self.mlp_fuse(torch.cat(feat_tmp, 0))
        return [coord, feat, offset]


# @MODELS.register_module("pointhr_partseg")
@MODELS.register_module("pointhrv2partv2")
class PointHR(nn.Module):
    def __init__(self,
        ...
        dec_channels = [enc_channels[-1]] + [enc_channels[-1]*(2**i) for i in range(len(enc_channels))]
        ...
        # self.dec_stages = Decoder(
        #     in_channels=dec_channels,
        #     groups=patch_embed_groups,
        #     depth=dec_depths[0],
        #     neighbours=patch_embed_neighbours,
        #     num_stream=enc_streams[-1],
        #     qkv_bias=attn_qkv_bias,
        #     pe_multiplier=pe_multiplier,
        #     pe_bias=pe_bias,
        #     attn_drop_rate=attn_drop_rate,
        #     drop_path_rate=dec_dp_rates[0],
        #     enable_checkpoint=enable_checkpoint,
        #     unpool_backend=unpool_backend
        # )
        if num_shape_classes is not None:
            self.cls_embed = ClsEmbed(dec_channels[-1], num_shape_classes)

        self.seg_head = nn.Sequential(
            nn.Linear(dec_channels[0], dec_channels[0]*4),
            PointBatchNorm(dec_channels[0]*4),
            nn.ReLU(inplace=True),
            nn.Linear(dec_channels[0]*4, num_classes)
    ...
    def forward(self, data_dict):
        ...
        points = x_list.pop(-1)
        if self.num_shape_classes is not None:
            points = self.cls_embed(points, cls)

        for i in reversed(range(1, self.num_stages)):
            skip_points, cluster = x_list.pop(-1), lookup.pop((i-1, i))
